...
export interface PumpRun {
  ...
  maxHeartrate?: number;
  startHeartrate?: number; // FC au début du tour
  endHeartrate?: number; // FC à la fin du tour
  startPosition?: [number, number];
  endPosition?: [number, number];
}

export interface PumpfoilSession {
  id: string;
...
export interface SessionStats {
  numberOfRuns: number;
  totalPumpingTime: number; // seconds
  totalPumpingDistance: number; // meters
...
export interface ProgressStats {
  ...
  totalRuns: number;
  totalPumpingTime: number;
  totalPumpingDistance: number;
...
export interface RunDetectionConfig {
  minSpeedThreshold: number; // km/h - speed above which we consider pumping
...
export interface AppState {
  ...
  selectedSession: PumpfoilSession | null;
