import { PumpfoilSession, StravaTokens, RunDetectionConfig } from '../types';

const STORAGE_KEYS = {
  TOKENS: 'pumpfoil_strava_tokens',
  SESSIONS: 'pumpfoil_sessions',
  CONFIG: 'pumpfoil_config',
  LAST_SYNC: 'pumpfoil_last_sync',
} as const;
...
interface StoredSession extends Omit<PumpfoilSession, 'rawData'> {
  rawData?: PumpfoilSession['rawData'] | CompressedRawData;
}

const compressRawData = (rawData: PumpfoilSession['rawData']): CompressedRawData | undefined => {
...
const decompressRawData = (data: CompressedRawData | PumpfoilSession['rawData']): PumpfoilSession['rawData'] => {
  ...
  return data as PumpfoilSession['rawData'];
};

const compressSession = (session: PumpfoilSession): StoredSession => {
  return {
    ...session,
...
const decompressSession = (stored: StoredSession): PumpfoilSession => {
  return {
    ...stored,
...
const DEFAULT_CONFIG: RunDetectionConfig = {
  minSpeedThreshold: 8, // km/h
  minRunDuration: 5, // seconds
  minStopDuration: 3, // seconds
...
export const addSession = (session: PumpfoilSession): void => {
...
export const migrateToCompressedStorage = (): { migrated: boolean; savedBytes: number } => {
  ...
  try {
    ...
    const compressed = parsed.map((s: PumpfoilSession) => compressSession(s));
...
export const getSessionById = (sessionId: string): PumpfoilSession | null => {
  const sessions = getSessions();
