...
// Pumpfoil activity types to look for - only Surf activities
const PUMPFOIL_ACTIVITY_TYPES = ['Surfing', 'Surf'];
...
export const getPumpfoilActivities = async (
  after?: number,
  before?: number
): Promise<StravaActivity[]> => {
  ...
  while (hasMore) {
    ...
    if (activities.length === 0) {
      ...
      // Filter for Surf activities only (pumpfoil sessions)
      const pumpfoilActivities = activities.filter(
        activity =>
          PUMPFOIL_ACTIVITY_TYPES.includes(activity.type) ||
          PUMPFOIL_ACTIVITY_TYPES.includes(activity.sport_type)
      );

      allActivities.push(...pumpfoilActivities);
      page++;
