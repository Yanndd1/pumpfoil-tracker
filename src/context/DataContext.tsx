...
import { PumpfoilSession, RunDetectionConfig, ProgressStats, StravaActivity } from '../types';
...
interface DataContextType {
  ...
  importActivity: (activityId: number) => Promise<PumpfoilSession | null>;
  deleteSession: (sessionId: string) => void;
  updateConfig: (config: Partial<RunDetectionConfig>) => void;
  reprocessSession: (sessionId: string) => Promise<void>;
  getSession: (sessionId: string) => PumpfoilSession | null;
}
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sessions, setSessions] = useState<PumpfoilSession[]>([]);
  ...
  const calculateProgressStats = (sessions: PumpfoilSession[]): ProgressStats => {
    const allRuns = sessions.flatMap(s => s.runs);
    const totalRuns = allRuns.length;
    const totalSessions = sessions.length;

    const totalPumpingTime = allRuns.reduce((sum, r) => sum + r.duration, 0);
    const totalPumpingDistance = allRuns.reduce((sum, r) => sum + r.distance, 0);
    ...
    return {
      ...
      totalRuns,
      totalPumpingTime,
      totalPumpingDistance,
      averageRunsPerSession: totalSessions > 0 ? totalRuns / totalSessions : 0,
      averageRunDuration: totalRuns > 0 ? totalPumpingTime / totalRuns : 0,
      averageRunDistance: totalRuns > 0 ? totalPumpingDistance / totalRuns : 0,
  ...
  const syncActivities = useCallback(async (): Promise<number> => {
    ...
    try {
      ...
      const activities = await stravaService.getPumpfoilActivities(after);
  ...
  const importActivity = useCallback(
    async (activityId: number): Promise<PumpfoilSession | null> => {
      if (!isAuthenticated) {
        setError('Not authenticated');
        return null;
      }
  ...
  const reprocessSession = useCallback(
    async (sessionId: string) => {
      ...
      const updatedSession: PumpfoilSession = {
        ...session,
        runs,
        stats,
      };
  ...
  const getSession = useCallback(
    (sessionId: string): PumpfoilSession | null => {
