...
const SessionsPage: React.FC = () => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        session =>
          ...
          format(new Date(session.date), 'MMMM yyyy', { locale: fr })
            .toLowerCase()
            .includes(query)
      );
    }
  ...
  const groupedSessions = useMemo(() => {
    ...
    filteredSessions.forEach(session => {
      const monthKey = format(new Date(session.date), 'MMMM yyyy', { locale: fr });
      if (!groups[monthKey]) {
        groups[monthKey] = [];
      }
  ...
  if (isLoading) {
    return (
      <Layout>
        <Loading message="Chargement des sessions..." />
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="space-y-6">
        ...
        {filteredSessions.length === 0 ? (
          <EmptyState
            title={sessions.length === 0 ? 'Aucune session' : 'Aucun résultat'}
            description={
              sessions.length === 0
                ? 'Synchronisez vos activités Strava pour voir vos sessions.'
