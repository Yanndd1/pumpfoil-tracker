import { PumpRun, RunDetectionConfig, SessionStats } from '../types';
...
/**
 * Detect pump runs from activity stream data
 *
...
/**
 * Create a PumpRun object from detected indices
 */
const createRun = (
  id: string,
  startIndex: number,
 ...
 */
export const calculateSessionStats = (runs: PumpRun[]): SessionStats => {
  if (runs.length === 0) {
    return {
      numberOfRuns: 0,
      totalPumpingTime: 0,
      totalPumpingDistance: 0,
  ...
  const totalPumpingTime = runs.reduce((sum, run) => sum + run.duration, 0);
  const totalPumpingDistance = runs.reduce((sum, run) => sum + run.distance, 0);
  ...
  return {
    ...
    averageRunDuration: totalPumpingTime / runs.length,
    averageRunDistance: totalPumpingDistance / runs.length,
